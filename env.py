...
class TactoEnv(gym.Env):
    def __init__(self, rl_cfg, digit_base=None):
        ...
        if p.getConnectionInfo()['isConnected'] == 0:
            # 如果没有连接，才init（首次），用GUI或DIRECT由你main.py控制
            #self._physics_client_id = px.init(mode='GUI')
            self._physics_client_id = px.init(mode=p.GUI)
        else:
            # 已有连接，直接用getConnectionInfo()['cid']或p.getConnectionInfo()返回的其它字段
            # 更通用的方式是p.getConnectionInfo()后啥都不用设置，PyBullet自己会复用全局连接
            self._physics_client_id = 0   # PyBullet默认主连接id就是0

        # 禁用PyBullet的图形界面
        p.configureDebugVisualizer(p.COV_ENABLE_GUI, 0)  # 禁用PyBullet的调试GUI界面
        ...
        self.digits.add_camera(self.digit_body.id, [-1]) # Add camera to the digit body
        ##### Workspace boundary #####
        ...
        # camera_thread = threading.Thread(target=self.rotate_camera)
        # camera_thread.start()
    ...
    def _compute_reward(self):
        ...
        if self.rl_cfg.reward.type == "AM":
            ...
            if self.no_touch_count >= self.rl_cfg.reward.no_touch_threshold:
                reward = self.rl_cfg.reward.no_contact_penalty 
            if self.action == 12:
                reward = self.rl_cfg.reward.no_contact_penalty 
            # print(
    ...
    def step(self, action):
        ...
        info["new_touch_points"] = self.pointcloud  # 推荐加这一行
    ...
    def reset_to_point(self, target_point, quat=[0,0,0,1]):
        """
        将digit手指/传感器移动到指定target_point，其他状态正常reset
        """
        # 例如：只需把digit_body移到target_point，并reset相关变量
        p.resetBasePositionAndOrientation(self.digit_body.id, target_point, quat)
        # ...你也可以复制reset()的内容，酌情处理
        self.curr_pos = np.array(target_point)
        # 其它变量/短期历史等初始化
        self.horizon_counter = 0
        self.no_touch_count = 0
        self.accumulated_reward = 0
